...
function extractFolderId(driveUrl) {
  const match = String(driveUrl || '').match(/\/folders\/([a-zA-Z0-9_-]+)/);
  if (match) return match[1];
...
function isSupportedMediaFile(file) {
  ...
  return supportedExtensions.has(extension) || /^(video|audio|image)\//.test(file.mimeType || '');
}

async function listFolderFiles(folderId, apiKey) {
  if (!apiKey) throw new Error('Google Drive API key is required. Add it in Settings first.');
  const files = [];
  let pageToken = '';
  do {
    ...
    files.push(...((response.data.files || []).filter(isSupportedMediaFile)));
...
async function tryDownloadFromUrl(url, cookies, commonHeaders) {
  ...
  const isVideo = !contentType.includes('text/html') &&
                  (contentType.includes('video') || contentType.includes('audio') || contentType.includes('image') ||
                   contentType.includes('octet-stream') || contentType.includes('application/'));
    ...
    return new Promise((resolve, reject) => {
      ...
      writer.on('finish', () => {
        try {
          ...
          if (!supportedExtensions.has(path.extname(finalOriginalFilename).toLowerCase())) {
            safeReject(new Error('This Google Drive file type is not supported by Gallery.'));
            return;
          }
