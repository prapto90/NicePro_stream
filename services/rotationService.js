...
const { generateYouTubeThumbnail } = require('../utils/videoProcessor');
    ...
    for (const rotation of activeRotations) {
      ...
      if (now >= scheduledEnd) {
        ...
        if (currentItem) {
          ...
          // Also stop a stream recovered after an application restart, when
          // the in-memory map has not been rebuilt yet.
...
async function startRotationStream(rotation, item) {
  try {
    ...
    // After an application restart the in-memory rotation map is empty, but
    // the managed FFmpeg encoder may still be live. Reuse that stream instead
    // of creating a second YouTube broadcast/encoder for the same item.
    const liveStreams = await Stream.findAll(rotation.user_id, 'live');
    const existingStream = liveStreams.find(candidate =>
      candidate.video_id === actualVideoId &&
      candidate.title === item.title &&
      streamingService.isStreamActive(candidate.id)
    );
    if (existingStream) {
      console.log(`[RotationService] Reusing recovered live stream ${existingStream.id} for rotation ${rotation.id}`);
      return { success: true, streamId: existingStream.id, recovered: true };
    }
    ...
    if (thumbnailToUpload) {
      ...
      try {
        // New Rotations selects an image directly from Gallery (a /uploads/... path),
        // while the legacy form stores only a filename. Support both safely.
        const publicRoot = path.join(__dirname, '..', 'public');
        const normalizedPath = String(thumbnailToUpload).replace(/\\/g, '/');
        const thumbnailPath = normalizedPath.startsWith('/uploads/')
          ? path.join(publicRoot, normalizedPath.substring(1))
          : path.join(publicRoot, 'uploads', 'thumbnails', path.basename(normalizedPath));
        if (fs.existsSync(thumbnailPath)) {
          const normalizedFilename = `youtube-${broadcast.id}-${Date.now()}.jpg`;
          normalizedThumbnailPath = await generateYouTubeThumbnail(thumbnailPath, normalizedFilename);
          await youtube.thumbnails.set({
            videoId: broadcast.id,
            media: {
              mimeType: 'image/jpeg',
              body: fs.createReadStream(normalizedThumbnailPath)
            }
          });
          console.log(`[RotationService] Uploaded thumbnail for broadcast ${broadcast.id}: ${path.basename(thumbnailPath)}`);
        } else {
          console.warn(`[RotationService] Thumbnail file not found: ${thumbnailPath}`);
        }
      } catch (thumbError) {
        console.error('[RotationService] Error setting thumbnail:', thumbError.message);
      } finally {
        if (normalizedThumbnailPath && fs.existsSync(normalizedThumbnailPath)) {
          fs.unlink(normalizedThumbnailPath, () => {});
        }
      }
    }
...
function shutdown() {
  ...
  // PM2/application restart must not stop managed FFmpeg. The maps are only
  // local bookkeeping and are reconstructed from verified live streams.
